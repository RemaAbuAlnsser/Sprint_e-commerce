...
export default function ParentCategoriesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingId
        ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/admin/parent-categories/${editingId}`
  ...
  const toggleActive = async (category: ParentCategory) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/admin/parent-categories/${category.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-foreground">الأقسام الرئيسية</h1>
            <p className="text-muted-foreground mt-2">إدارة الأقسام الرئيسية للمتجر</p>
          </div>
          <button
        ...
        {showForm && (
          <div className="bg-card p-6 rounded-lg border border-border mb-8">
            <h2 className="text-xl font-bold mb-4">
              {editingId ? 'تعديل القسم الرئيسي' : 'إضافة قسم رئيسي جديد'}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-2">رابط صورة البنر (تظهر في الصفحة الرئيسية أعلى منتجات القسم)</label>
                <input
                  type="text"
                ...
                {formData.banner_image && (
                  <div className="mt-2 rounded-lg overflow-hidden border border-border">
                    <img src={formData.banner_image} alt="معاينة البنر" className="w-full h-40 object-cover" />
                  </div>
                )}
              </div>
        ...
        <div className="bg-card rounded-lg border border-border overflow-hidden">
          <table className="w-full">
            <thead className="bg-muted">
              <tr>
                ...
                <th className="px-6 py-4 text-right text-sm font-semibold">اسم القسم</th>
                <th className="px-6 py-4 text-right text-sm font-semibold">المعرف</th>
                <th className="px-6 py-4 text-right text-sm font-semibold">الأقسام الفرعية</th>
                <th className="px-6 py-4 text-right text-sm font-semibold">المنتجات</th>
            ...
            <tbody className="divide-y divide-border">
              {parentCategories.map((category) => (
                <tr key={category.id} className="hover:bg-muted/50">
                  ...
                  <td className="px-6 py-4 text-sm">{category.subcategories_count || 0}</td>
                  <td className="px-6 py-4 text-sm">{category.products_count || 0}</td>
                  <td className="px-6 py-4">
                    <button
          ...
          {parentCategories.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              لا توجد أقسام رئيسية. قم بإضافة قسم جديد للبدء.
            </div>
          )}
        </div>
      </div>
      <ConfirmModal
        ...
        message={`هل أنت متأكد من حذف "${confirmDelete.name}"؟ سيتم حذف جميع الأقسام الثانوية المرتبطة به.`}
        confirmText="حذف"
        cancelText="إلغاء"
        type="danger"
